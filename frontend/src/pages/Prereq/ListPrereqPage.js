...
export default function ListPrereqPage(){
    ...
    function getPrereqs() {
        axios.get('http://127.0.0.1:5000/admin/prereqs').then(function(response) {
    ...
    const deletePrereq = (id) => {
        axios.delete(`http://127.0.0.1:5000/admin/prereqs/delete/${id}`).then(function(response){
            console.log(response.data);
            getPrereqs();
        });
                        ...
                        <tbody>
                            {prereqs.map((prereq,key) =>
                                <tr key = {key}>
                                    ...
                                    <Link to={`/prereqs/prereq/${prereq.prereq_id}/edit`} className="btn btn-success" style={{marginRight: "10px"}}>Edit</Link>
